...
public class OVXFlowMod extends OVXMessage implements Devirtualizable {
    ...
    public void devirtualize(final OVXSwitch sw) {
        ...
        virtualCookie = ovxMatch.getCookie();
        ...
        PhysicalFlowTable pft = ovxInPort.getPhysicalPort().getParentSwitch().getPhysicalFlowTable();
        //log.info("switch {} pft {}\n", ovxInPort.getPhysicalPort().getParentSwitch().getName(), ovxInPort.getPhysicalPort().getParentSwitch().getPhysicalFlowTable().toString());

        short outPortNum = 0;
        for (final OFAction act : this.getFlowMod().getActions()) {
            /* ksyang: For port monitoring */
            if(act.getType()==OFActionType.OUTPUT){
                OFActionOutput outputAction = (OFActionOutput)act;
                outPortNum = outputAction.getPort().getShortPortNumber();
                final OVXPort ovxOutPort = sw.getPort(outPortNum);
                //log.info("Ports going into PCookiePortMap - inport num {} portnum {} outport num {} portnum {}", inport, ovxInPort, outPortNum, ovxOutPort);

                log.info("Now cookie {}, {} - exist ? {}", virtualCookie, this.getFlowMod().getCookie().getValue(), pft.getInPortFromPCookie(this.getFlowMod().getCookie().getValue()) == null);
                log.info("inport {} outport {}", ovxInPort.getPortNumber(), ovxOutPort.getPortNumber());

                if(pft.getInPortFromPCookie(this.getFlowMod().getCookie().getValue()) == null || pft.getInPortFromPCookie(this.getFlowMod().getCookie().getValue()) == null) {
                    if (ovxInPort != null && ovxOutPort != null) {
                        pft.addPCookiePortMap(this.getFlowMod().getCookie().getValue(), ovxInPort, ovxOutPort);
                    }
                }
    ...
    private void prepAndSendSouth(OVXPort inPort, boolean pflag) {
        ...
        if (pflag) {
            ...
            // ksyang
            PhysicalFlowTable pft = inPort.getPhysicalPort().getParentSwitch().getPhysicalFlowTable();
            //log.info("Processing new flow rule {} to pft : {}", this.getFlowMod().getCookie().toString(), pft.handleFlowMods(this, virtualCookie));
            pft.handleFlowMods(this, virtualCookie);
